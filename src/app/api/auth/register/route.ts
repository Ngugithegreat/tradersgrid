...
export async function POST(req: Request) {
  try {
    ...
    const existing = (await sql`SELECT id FROM tradersgrid_users WHERE email = ${cleanEmail} LIMIT 1`) as any[];
    if (existing.length) {
      return NextResponse.json(
    ...
    const anyUser = (await sql`SELECT 1 FROM tradersgrid_users LIMIT 1`) as any[];
    ...
    if (ref) {
      ...
      if (rid) {
        const r = (await sql`SELECT id FROM tradersgrid_users WHERE id = ${rid} LIMIT 1`) as any[];
        if (r.length) referredBy = rid;
      }
    }

    const rows = (await sql`
      INSERT INTO tradersgrid_users (name, email, password_hash, role, balance, country, phone, referred_by)
